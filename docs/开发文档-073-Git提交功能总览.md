...
>
> 📦 **图文放一起的版本**（原文 + 4 张截图在一个文件/一个包里，方便直接发给别人）：
> - `docs/git-overview/Git提交功能总览-图文版.html` —— 单文件，图片已 base64 内联，双击用浏览器打开即可
> - `docs/git-overview/Git提交功能总览-图文包.zip` —— `Git提交功能总览.md` + 上面那个 html + `img/` 4 张原图
>
> 生成方式：改完本文后重跑 `node <临时脚本> md2html` 那套（把 md 里的 `![](img/x.png)` 内联成 data URI），再打包；
