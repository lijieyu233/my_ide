...
>
> 📦 **图文放一起的版本**（文档 + 截图在一个文件/一个包里，方便直接发给别人）：
> - `docs/git-overview/Git文档-图文版.html` —— 单文件，**073 现状 + 074 路线图 + 075/076/077（M1/M2/M3 已落地）**
>   五篇都在里面，图片已 base64 内联，双击用浏览器打开即可
> - `docs/git-overview/Git提交功能总览-图文包.zip` —— 上面五篇 md + `待办-提交窗口剩余项.md` + 那个 html + `img/` 7 张原图
>
> 生成方式：改完 md 后重跑内联脚本（把 `![](img/x.png)` 换成 data URI 再拼成一份 HTML），再用 Python `zipfile` 打包；
> 图片本身由 `electron . --check-ui` 产出后拷进 `docs/img/`。
...
否则自检/单测跑不起来。isomorphic-git **没有** `add -p` / `merge` / `rebase` / hooks。

> ⚠ **一处更正（2026-09-23）**：本文早先写"也没有 credential helper"是**错的**。
> `git-service.js:940-960` 的 `systemCredentialFill()` 已经在用 `execFile('git', ['credential','fill'])`
> 读**系统 Git 凭证**（按 protocol//host 进程内缓存），`:884` 还用 `git config --get-urlmatch http.proxy` 拿代理
> —— 也就是说**原生 git 早就在被 spawn 了**；按主机存的 `myide-git-auth-map` 只是没有 helper 时的兜底 UI。
> 后续规划见 `docs/开发文档-074-Git下一阶段路线图（评审回应）.md`。
