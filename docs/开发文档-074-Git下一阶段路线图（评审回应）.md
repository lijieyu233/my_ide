...
>
> ✅ **收尾已完成**（2026-09-23）：见 `docs/开发文档-079-M4收尾-拉取策略与安全强推与可编辑合并.md`。
> Pull 四策略（含拉取并合并 / 拉取并变基）、`--force` → `--force-with-lease`、冲突窗口加**可编辑合并结果**
> （起始 = 带冲突标记的工作区版本）、分支弹窗加远程分支段 + upstream。
> ⚠ 该篇 §1 记了一个环境级数据损失缺陷：本机 `git rebase` 的 merge 后端会删掉整个 `.git`，已强制 apply 后端。
> 仍未做：PyCharm 式三栏联动可编辑合并器；remote-tracking 引用在本机不落地（环境缺陷）导致
> lease 的基线可能永不更新、push 的 ahead/behind 依赖 fetch —— 已记录在案。
...
> ✅ **第一批已完成**（2026-09-23）：见 `docs/开发文档-080-M5-A-提交前检查与Sign-off与作者覆盖.md`。
> 提交前检查（自定义命令 + `git hook run pre-commit` 同一套执行器、失败即停、结果面板、消息正则/长度校验、
> TODO 扫描只提示不阻断）+ Sign-off（幂等追加）+ 作者覆盖（不写 git config）。
> **明确不做**：Reformat / Rearrange / Optimize imports / Analyze code（要接入四套 lint/format 工具链，
> 等于重做 IDE 的 lint 层 —— 在 UI 与文档里都写明了不做，避免交半成品）。
> 仍未做：Local History（AI checkpoint）· Git Log 三栏 · 工具行 `⋮ More` · 高级 Git（interactive rebase / reset /
> worktree / submodule / multi-root / signing）· 待办第 5 节的作者下拉已顺手做完、第 6 节小项未动。
