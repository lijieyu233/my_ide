...
**浏览与编辑**
- 文件树：懒加载、展开状态记忆、**虚拟滚动**（大目录不卡）、**打开文件自动定位高亮**、**Git 状态着色**（新增绿/修改蓝/删除红）、**一键收起/展开全部目录**、**目录区宽度可拖拽调整**（持久化）、**树内拖拽移动文件**、**Ctrl+Z 撤销文件操作**（粘贴/新建/重命名/删除/移动）
- 多标签：中键关闭、右键关闭其他/全部、**拖拽排序**、**标签过多自动收缩 + 「▾ 全部」下拉**、会话记忆（每个项目独立）
- 编辑器：**行号**、查找/替换（Ctrl+F / Ctrl+H）、**括号配对自动补全**、**字号缩放（Ctrl+=/-）**、**GBK/UTF-16（含无 BOM）编码自动识别与回写**、**自动保存**（停止输入 3 秒写盘）
- 新建文件/文件夹（右键菜单）、重命名、删除（可撤销）、**文件复制粘贴（与资源管理器互通，CF_HDROP 格式）**
- 快速打开（Ctrl+P）无输入时显示**最近打开的文件**
- **单击文件只打开不复制路径**（复制用 Ctrl+Shift+C 或悬停「复制路径」）；点击文件树**不再上下跳动**（目标行不可见时才滚动）

**渲染（插件机制，可扩展任意格式）**
- Markdown（代码高亮、**默认「编辑 + 实时预览」分屏并存**、wiki 链接 `[[笔记]]` 跳转、远程/相对图片）、HTML（沙箱预览 + **🌐 浏览器打开**，iframe 内快捷键不失效）、CSV 表格、**图片、PDF、视频、音频**（内置查看器，图片纯色背景无格子）
...
- 本地修改**按目录分组折叠、只显示文件名**（完整路径悬浮显示）、**点击直接打开文件**、↔ 查看对比、**↺ 放弃修改（revert）**；状态栏**点击分支切换、点击路径复制**
...
**多项目**：顶部项目栏打开/切换多个项目（**可拖拽排序、✕ 关闭**），每个项目独立标签记忆

**窗口**：**去掉 Windows 原生标题栏**，自绘顶栏（可拖拽窗口、─ □ ✕ 窗口控制）
