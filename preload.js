...
contextBridge.exposeInMainWorld('myIDE', {
  ...
  git: {
    ...
    // M4：分支工作流 / 冲突解决（原生 git；无本机 git 时返回值带 error，UI 按 caps 隐藏入口）
    opState: (d) => ipcRenderer.invoke('git:opState', d),
    merge: (d, ref, o) => ipcRenderer.invoke('git:merge', d, ref, o),
    rebase: (d, ref) => ipcRenderer.invoke('git:rebase', d, ref),
    conflicts: (d) => ipcRenderer.invoke('git:conflicts', d),
    conflictSides: (d, f) => ipcRenderer.invoke('git:conflictSides', d, f),
    resolveFile: (d, f, side) => ipcRenderer.invoke('git:resolveFile', d, f, side),
    continueOp: (d) => ipcRenderer.invoke('git:continueOp', d),
    skipOp: (d) => ipcRenderer.invoke('git:skipOp', d),
    abortOp: (d) => ipcRenderer.invoke('git:abortOp', d),
    branchCreate: (d, name, ref, co) => ipcRenderer.invoke('git:branchCreate', d, name, ref, co),
    branchRename: (d, from, to) => ipcRenderer.invoke('git:branchRename', d, from, to),
    branchDelete: (d, name, force) => ipcRenderer.invoke('git:branchDelete', d, name, force),
    resolveCustom: (d, f, text) => ipcRenderer.invoke('git:resolveCustom', d, f, text),
    readWorktreeText: (d, f) => ipcRenderer.invoke('git:readWorktreeText', d, f),
